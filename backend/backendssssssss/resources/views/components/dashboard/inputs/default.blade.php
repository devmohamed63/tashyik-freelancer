<div>
    ...
    <div class="relative">
        <input
            {{ $attributes->only(['placeholder', 'required', 'wire:model', 'wire:model.live', 'min', 'step', 'autocomplete', 'x-bind:id', 'x-bind:value'])->merge(['name' => isset($locale) ? "{$name}[{$locale}]" : $name]) }}
            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 @error(isset($locale) ? "{$name}.{$locale}" : $name) input-error @else @if (old(isset($locale) ? "{$name}.{$locale}" : $name)) input-success @endif @enderror"
            type="{{ $type ?? 'text' }}"
            id="{{ $id ?? $name }}"
            value="{{ isset($locale) ? old("{$name}.{$locale}", $value ?? null) : old($name, $value ?? null) }}" />

        @error(isset($locale) ? "{$name}.{$locale}" : $name)
        ...
        @else
            @if (isset($locale) ? old("{$name}.{$locale}") : old($name))
                <span class="absolute top-1/2 end-3.5 -translate-y-1/2">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
    ...
    @isset($description)
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-500">{{ $description }}</p>
    @endisset
