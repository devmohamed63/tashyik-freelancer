...
class GenerateSeoArticleForServiceJob implements ShouldQueue
{
    ...
    public function handle(GeminiBlogGenerator $generator): void
    {
        ...
        if (!$this->force && !$this->recycle && $this->serviceAlreadyHasAiArticleThisMonth($service->id)) {
            Log::info('GenerateSeoArticleForServiceJob: skipped — service already has an AI article this month', [
        ...
        if ($imagePrompt === '') {
            ...
            if ($tmp) {
                ...
                Log::warning('GenerateSeoArticleForServiceJob: featured image not generated (OpenAI + Pollinations failed or timed out)', [
                    'article_id' => $article->id,
                    'service_id' => $service->id,
